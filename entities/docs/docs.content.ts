        ...
        blocks: [
            ...
            },
            {
                ...
                items: [
                    ...
                    '**Phase 5 (e2e)**: Live tests written (2-layer pipeline with ollama-cloud→oMLX, rewind, resume) but not executed (requires keys + oMLX server). Marked `#[ignore]` + env gates (`STEPPER_E2E`).',
                    '**Phase 6 (real use)**: Real LLM streaming, real tty TUI, Codex backend, `/init` polish, rewind/resume user experience.',
                    '**Implemented (B/C features)**: Dispatch tool, model-driven compaction, handoff retry/skip, `alwaysLoad`, context window override, slash command palette.',
                    '**Deferred (accepted limits)**: WriteFile TOCTOU symlink swap (single-user dev CLI scope), birdcage OS sandbox (hermetic testing infeasible), gix checkpoint backend (copy works), keyring native library hermetic (keychain unavailable in test), MCP HTTP auth live testing, `McpManager::connect` success arm hermetic (live server required).',
                ],
            },
        ],
    },
