...
export const CommandBlock: FC<CommandBlockProps> = ({ command, prompt = '$', copyLabel, copiedLabel, className }) => (
    <div className={cn('flex w-full min-w-0 items-center gap-3 rounded-md border bg-card px-4 py-3 font-mono text-sm shadow-xs', className)}>
        <span aria-hidden className='shrink-0 select-none text-muted-foreground'>
            {prompt}
        </span>
        <code className='min-w-0 flex-1 overflow-x-auto whitespace-nowrap text-card-foreground'>{command}</code>
